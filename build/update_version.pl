...
# win32/Configure.pl
# my $VERSION = '[^']+'
my $pattern3 = qr/my \$VERSION = "2.18"/;
my $replace = "my \\\$VERSION = \"$version\"";
system "perl -i -ple 's/$pattern3/$replace/' win32/Configure.pl";

# RELEASE/WEBSITE/this script
system "perl -i -ple 's/2.18/$version/' build/RELEASE";
system "perl -i -ple 's/2.18/$version/' build/WEBSITE";
system "perl -i -ple 's/2.18/$version/' build/update_version.pl";
